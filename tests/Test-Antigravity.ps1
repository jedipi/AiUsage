...
try {
    ...
    if ($result.antigravity.fiveHour.usedPercent -ne 72) { throw "Expected 72% five-hour remaining, got $($result.antigravity.fiveHour.usedPercent)." }
    if ($result.antigravity.weekly.usedPercent -ne 84) { throw "Expected the lowest weekly quota, got $($result.antigravity.weekly.usedPercent)." }
    if ($result.antigravity.weekly.resetAt -notmatch '^\d{4}-\d{2}-\d{2}T') { throw 'Weekly reset time was not normalized to ISO.' }
    $flat = Get-Content -Raw -LiteralPath (Join-Path $testCache 'usage.cache')
    if ($flat -notmatch 'antigravity\.fiveHour\.usedPercent=72') { throw 'Antigravity five-hour cache value is missing.' }
    if ($flat -notmatch 'antigravity\.weekly\.usedPercent=84') { throw 'Antigravity weekly cache value is missing.' }
    ...
    if ($statusLineOutput -notmatch 'TokenMeter:.*gemini-five-hour 72%') { throw 'Antigravity status-line adapter output is incorrect.' }
